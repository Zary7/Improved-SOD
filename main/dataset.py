...
class Data(Dataset):
    def __init__(self, cfg, model_name):
        ...
        with open(cfg.datapath + '/' + cfg.mode + '.txt', 'r') as lines:
            raw_samples = [line.strip() for line in lines]

        # 用 PIL 验证样本可读性（掩码文件夹已改为 GT）
        valid_samples = []
        print(f"Total samples in txt: {len(raw_samples)}")
        for name in raw_samples:
            img_path = cfg.datapath + '/Imgs/' + name + '.jpg'
            try:
                with Image.open(img_path) as img:
                    img.verify()
            except Exception as e:
                print(f"⚠️  Removing bad sample (image unreadable): {name} ({e})")
                continue
            if cfg.mode == 'train':
                # 掩码文件夹由原来的 Masks 改为 GT
                mask_path = cfg.datapath + '/GT/' + name + '.png'
                try:
                    with Image.open(mask_path) as msk:
                        msk.verify()
                except:
                    print(f"⚠️  Removing bad sample (mask unreadable): {name}")
                    continue
    ...
    def __getitem__(self, idx):
        ...
        img_path = self.cfg.datapath + '/Imgs/' + name + '.jpg'
        ...
        if self.cfg.mode == 'train':
            # 掩码文件夹改为 GT
            mask_path = self.cfg.datapath + '/GT/' + name + '.png'
